...
export const pullConnection = async (connectionID, setConnection, userID) => {
  ...
  const subConnections = firestore.collection(
    `connections/${connectionID}/userData/`
  );
  if (userID) {
    try {
      await user.update({
...
export const deleteConnectionFromFirebase = async (
  connection,
  currentUserID
) => {
  const userRef = firestore.doc(`users/${currentUserID}`);
...
export const signInWithFacebook = () =>
  authFB.signInWithPopup(providerFacebook);
